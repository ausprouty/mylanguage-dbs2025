...
export async function getContentWithFallback({
  ...
  if (typeof storeGetter !== "function") {
    throw new TypeError(
      "getContentWithFallback: storeGetter must be a function"
    );
  }
  if (typeof storeSetter !== "function") {
    throw new TypeError(
      "getContentWithFallback: storeSetter must be a function"
    );
  }
  ...
  if (!apiUrl || typeof apiUrl !== "string" || apiUrl.trim() === "") {
    throw new TypeError(
      "getContentWithFallback: apiUrl must be a non-empty string"
    );
  }
  if (!translationType || typeof translationType !== "string") {
    throw new TypeError(
      "getContentWithFallback: translationType must be a string"
    );
  }
  ...
  try {
    ...
    const complete =
      skipTranslationCheck || isTrueFlag(data?.language?.translationComplete);
